...
import math
import rospy
from race.msg import pid_input, AngleDist
...
def control(data):
	...
	# 2. Apply the PID equation on error to compute steering
        ...
        #angle = theta / (math.pi / 2) * 100
	angle = 1.5*theta * 180 / math.pi
        if angle > 100:
		angle = 100
	elif angle < -100:
		angle = -100

	print("OG Angle: " + str(angle))
	prev_error = data.pid_error
        # vel = max(5, 25 - (abs(angle)))
	...
	command.speed = vel #vel

	# Move the car autonomously
	command_pub.publish(command)
	angle_graph = 90
	arrow_marker = create_steering_arrow(0, 0, 0, theta)
	marker_pub.publish(arrow_marker)
        pub_arrow.publish(AngleDist(theta, 2, -1, "yellow"))
