...
def getRange(data):
	...
	for i in range(kmin, kmax):
		...
		print(alpha, beta)
		if dist1 >= dist2:
			left = idx_to_angle(angle1 - alpha, data) - kmin
			left = max(left, 0)
			right = i1 - kmin + 1
		else:
			left = i2 - kmin
			right = idx_to_angle(angle2 + beta, data) - kmin
			right = min(right, len(overall_dists))
		print(left, right, len(overall_dists))
...
# navigating to the center of the widest gap
	...
	for i in greater_than_thresh[1:]:
		len_gap = i - prev
		if len_gap > largest_len:
			largest_len = len_gap
			largest_len_center =  (i + prev) // 2
		prev = i
